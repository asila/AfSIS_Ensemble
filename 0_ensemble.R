...
#som_mir <- predict(mir.som, mirv) ## predict validation set

#som_mir.a <- predict(mir.som,mir.a)
#som_mir.a <- predict(mir.som, mirv) ## predict validation set; replace with mir.a

# Save the model 

#saveRDS(mir.som, file = paste0(ref.hd[q],"_SOM.RDS"))
...
pmirv <- as.data.frame(cbind(lv, rfo_mir, xgb_mir, pls_mir,cubist_mir)) #bar_mir


pmirv.a <- as.data.frame(cbind(rfo_mir.a, xgb_mir.a, pls_mir.a,cubist_mir.a)) #bar_mir.a,

names(pmirv) <- c("L", "RFO", "XGB", "PLS","CUBIST")

names(pmirv.a) <- c("RFO", "XGB", "PLS","CUBIST")
